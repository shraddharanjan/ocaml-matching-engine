...
let no_crossed_book engine =
  ...
  match
    ...
    Order_book.best_ask book
  with
  | Some (best_bid, _), Some (best_ask, _) ->
      Price.compare best_bid best_ask < 0
  | _ ->
      true
;;

let total_trade_quantity trades =
  List.sum
    (module Int)
    trades
    ~f:Trade.quantity
...
let valid_price_generator =
  QCheck.(0 -- 1_000_000)
;;

let valid_quantity_generator =
  QCheck.(1 -- 10_000)
;;
...
let run_crossing_case
  ...
  =
  let resting_side =
    Order.Side.opposite incoming_side
  in
...
let crossing_orders_trade =
  QCheck.Test.make
    ...
    (fun
      ...
      , ( base_ticks
        , ( improvement
          , (resting_quantity, incoming_quantity) ) ) )
    ->
      match
        run_crossing_case
          incoming_side
      ...
      | Error _ ->
          false
      | Ok (engine, trades) ->
          let expected_trade_quantity =
            Int.min resting_quantity incoming_quantity
          in

          List.length trades = 1
          && total_trade_quantity trades
             = expected_trade_quantity
          && no_crossed_book engine)
;;
  ...
  QCheck.Test.make
    ...
    ->
      ...
      | Ok (engine, trades) ->
          ...
          let resting_quantity_after =
            remaining_quantity engine
          in

          submitted_quantity
          = (2 * executed_quantity)
            + resting_quantity_after)
;;
...
let crossing_orders_leave_correct_count =
  QCheck.Test.make
    ...
    ->
      ...
      | Ok (engine, _) ->
          let expected_count =
            if resting_quantity = incoming_quantity
            then 0
            else 1
          in
      ...
      | Ok (engine, _) ->
          (match Engine.cancel engine id with
           ...
           | Ok (engine, cancelled_order) ->
               ...
               && Order.Side.equal
                    (Order.side cancelled_order)
                    side
               && Price.equal
                    (Order.price cancelled_order)
                    price
               && Order.quantity cancelled_order
                  = quantity
...
let invalid_quantity_generator =
  QCheck.((-10_000) -- 0)
;;
    ...
    (fun quantity ->
      match
        Engine.submit_limit_order
          ...
          ~quantity
      with
      | Error (Engine.Invalid_quantity returned_quantity) ->
...
let crossing_is_deterministic =
  QCheck.Test.make
    ...
    ->
      ...
      | Ok (first_engine, first_trades),
        Ok (second_engine, second_trades) ->
          Sexp.equal
            (Engine.sexp_of_t first_engine)
            (Engine.sexp_of_t second_engine)
          && List.equal
               Trade.equal
               first_trades
               second_trades
...
let () =
  Alcotest.run
    ...
    ; ( "cancellation"
      , to_alcotest
          [ submitted_order_can_be_cancelled
          ] )
    ]
;;