...
let rec match_order engine incoming trades =
  ...
  | None ->
      let updated_book =
        Order_book.add engine.book incoming
      in

      { engine with book = updated_book },
      List.rev trades
      ...
      then (
        ...
        let engine =
          { engine with
            book = updated_book
          ; next_trade_sequence =
              engine.next_trade_sequence + 1
          }
        in

        if incoming_quantity = 0
        then
          engine, List.rev (trade :: trades)
        else (
          let remaining_incoming =
            Order.with_quantity
              incoming
              incoming_quantity
          in
  ...
  then Error (Duplicate_order_id id)
    ...
    let order =
      Order.create
        ...
        ~price
        ~quantity
        ~sequence:engine.next_order_sequence
    in

    let engine =
      { engine with
        next_order_sequence =
          engine.next_order_sequence + 1
      }
    in

    let engine, trades =
      match_order engine order []
    in
...
let cancel engine id =
  let updated_book, removed =
    Order_book.cancel engine.book id
  in

  match removed with
  | None ->
      Error (Unknown_order_id id)

  | Some order ->
      Ok ({ engine with book = updated_book }, order)
;;