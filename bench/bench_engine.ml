...
let benchmark_full_match =
  Bench.Test.create
    ~name:"full match against one resting order"
    (fun () ->
      Engine.submit_limit_order
...
let benchmark_partial_match =
  Bench.Test.create
    ~name:"partial match against one resting order"
    (fun () ->
      Engine.submit_limit_order
        engine_with_resting_sell
        ~id:(Id.of_string "buy-1")
...
let build_same_price_book order_count =
  List.fold
    (List.range 0 order_count)
    ~init:Engine.empty
    ~f:(fun engine index ->
      let id =
        Id.of_string (sprintf "sell-%d" index)
      in
...
let benchmark_add_same_price_orders =
  Bench.Test.create
    ~name:"add 1,000 orders at one price"
    (fun () ->
      build_same_price_book 1_000
      |> Sys.opaque_identity
      |> ignore)
;;

let engine_with_many_sells =
  build_same_price_book 1_000
;;

let benchmark_sweep_same_price_level =
  Bench.Test.create
    ~name:"sweep 1,000 resting orders"
    (fun () ->
      Engine.submit_limit_order
